...
describe("OpenIdClient", () => {
  ...
  it("handles code exchange, userinfo and introspection", async () => {
    ...
    const token = await Effect.runPromise(client.exchangeAuthorizationCode({ code: "code" }));
    expect(token.access_token).toBe("access-token");

    const user = await Effect.runPromise(client.fetchUserInfo(token.access_token));
    expect(user.sub).toBe("123");

    const introspected = await Effect.runPromise(client.introspectToken(token.access_token));
    expect(introspected.active).toBe(true);

    await Effect.runPromise(client.revokeToken(token.access_token));
    expect(await Effect.runPromise(client.endSessionUrl({ state: "bye" }))).toContain("state=bye");
  });
});
